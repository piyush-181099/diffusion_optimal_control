...
class MFDDP(DDP):
    ...
    """
    def __init__(self, *args, k_mf=64, update_mode='adam', lr=1e-3, **kwargs):
    ...
    def adam(self, states, actions, ks, qTKs, qs, iter_num, last=False):
        ...
        if self.optimizers is None:
            ...
            self.optimizers = [torch.optim.Adam([action], lr=self.lr) for action in actions]
    ...
    def compute_gradients(self, states, actions):
        ...
        for t in range(n_steps - 1, -1, -1):
            ...
            # fxT_Vx, fuT_Vx = torch.func.vjp(fn, state, action)[1](Vx.reshape(d_b, d_s))
            # q, qTfx, qTfu = Vx.reshape(d_b, d_s, 1) / Vx.norm(), fxT_Vx.reshape(d_b, 1, d_s) / Vx.norm(), fuT_Vx.reshape(d_b, 1, d_s) / Vx.norm()
            ...
            qvTK = 0 * (-0.5) * qvTQux / (luu_scalar + self.eps) + qv.mT @ qu @ quTinv @ qv @ qvTQux
