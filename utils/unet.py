...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.h_dim = (out_ch, image_size // 2 ** (len(channel_mult) - 1), image_size // 2 ** (len(channel_mult) - 1))
    ...
    def forward(self, x, timesteps, y=None, u=None):
        ...
        for module in self.input_blocks:
            ...
            hs.append(h)
        if u is not None:
            assert u.shape == h.shape, f"u.shape: {u.shape} and h.shape: {h.shape} are different!"
            h = h + u
